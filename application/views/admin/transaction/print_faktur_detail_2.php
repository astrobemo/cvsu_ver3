...
function print_kombinasi_2(printer_name){
	...
	<?
		if ($baris_idx > 20 ) { ?>
			...
			var data = ['\x1B' + '\x40'+          // init
		   		...
		   		<?="'".sprintf('%-64.64s', 'NOTE :  Harga sudah incl. 10% PPN '.$keterangan1 )."'";?>+'\x09'+
			   	'\x1B' + '\x21' + '\x01'+ // em mode on
				...
				<?="'".sprintf('%-6.6s', '')."'";?>+ '\x09'+
				<?="'".sprintf('%-13.13s', '')."'";?>+ 
				<?="'".sprintf('%-14.14s', number_format($total,'2',',','.'))."'";?>+ '\x09'+
				...
				<?="'".sprintf('%36.36s', number_format($total - $diskon,'2',',','.'))."'";?> + 
			   	...
			   	foreach ($data_pembayaran as $row) {?>
					...
					<?="'".sprintf('%36.36s', number_format($row->amount,'2',',','.'))."'";?> + 
				   	'\x0A'+
			   	<?}
			   	for ($idx_bayar=0; $idx_bayar < 3 ; $idx_bayar++) { ?>
			   		'\x0A'+
			   	<?}
			   	?>
