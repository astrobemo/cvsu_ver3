							...
							<tbody>
								...
								foreach ($barang_list as $row) { 
									...
									<tr>
										...
										<td>
											<span class='harga_beli'><?=number_format($row->qty,'2','.','.');?></span> 
											<?=$row->nama_satuan;?>
										</td>
										<td>
											<?=$row->jumlah_roll;?>
										...
										<td>
											<a href="<?=base_url().is_setting_link('report/barang_masuk_list_detail_report');?>?toko_id=<?=$toko_id;?>&supplier_id=<?=$supplier_id?>&tanggal_start=<?=$tanggal_start;?>&tanggal_end=<?=$tanggal_end;?>&barang_id=<?=$row->barang_id;?>&warna_id=<?=$row->warna_id;?>" class='btn btn-xs yellow-gold' onclick="window.open(this.href, 'newwindow', 'width=1250, height=650'); return false;"><i class='fa fa-search'></i></a>
										</td>
									</tr>
								<? 
								$idx++;
