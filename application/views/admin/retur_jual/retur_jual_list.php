...
#add-select-penjualan-container{
	padding: 5px;
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<!-- <form action="<?=base_url('retur_jual/retur_jual_list_insert')?>" class="form-horizontal" id="form_add_data" method="post"> -->
							...
							<div class="form-group">
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-6">
									...
									<input type="text" id="penjualanId" name="penjualan_id" hidden>

									<div id="penjualanListContainer" class="show-list-active">
										<label class="form-control" id="selectedPenjualan" onclick="showList()">Pilih</label>
										<div id='add-select-penjualan-container' style="position: relative;">
											<input type="text" id="penjualanFilter" class="form-control">
											<p id="penjualanLoad"><i class="fa fa-cog fa-spin"></i></p>
											<ul id='penjualanList'>
								...
								<?foreach ($retur_list as $row) { ?>
									<tr>
										...
										<td  class='text-center'>
											<?foreach ($qty as $key => $value) {
												echo number_format(($harga[$key] * $qty[$key])-$row->potongan_harga,'0',',','.').'<br>';
											}?>
										</td>
										<td >
											<?=($row->retur_type_id != 3 ? $row->nama_customer : $row->nama_keterangan);?>
										</td>
										<td>
											<a class='btn btn-xs green' target='_blank' href="<?=base_url().is_setting_link('retur_jual/retur_jual_detail');?>?id=<?=$row->id;?>"><i class='fa fa-edit'></i></a>
											<?if ($row->status_aktif == 1 && is_master_admin()) {?>
...
var select = null;
const penjualanFilter = document.querySelector("#penjualanFilter");
const selectedPenjualan = document.querySelector("#selectedPenjualan")
var penjualanList = [];
var penjualanListFiltered = [];
var isFilterMore = false;
...
jQuery(document).ready(function() {
	...
	$('.btn-save').click(function(){
		...
		var customer_id = $('#form_add_data [name=customer_id]').val();
		...
		if (tanggal != '') {
			if (retur_type_id == 1) {
				if (customer_id != '') {
					...
					notific8("ruby", "Mohon isi customer data");
				};
			}else{
				$('#form_add_data [name=customer_id]').val('');
				$('#form_add_data').submit();
			};
		}else{
	...
	penjualanFilter.addEventListener('keyup', debounce(function(event) {
		filterPenjualan();
	}, 300));
});
...
<?//if (is_posisi_id() <= 3) {?>
	...
	async function updateStatRetur(id, status){
		const response = await fetch(baseurl+"retur_jual/retur_jual_list_batal",{
			method:"POST",
			headers:{
	...
	async function get_penjualan_list(){
		var bulkyOptions = [];

		const response = await fetch(baseurl+"retur_jual/get_penjualan_list_retur",{
			method:"GET"
		});
		penjualanList = await response.json();
		penjualanListFiltered = penjualanList;
		filterPenjualan();
	}

	function listPenjualanDraw(){
		const pL = document.querySelector("#penjualanList");
		$("#penjualanList").html("");
		$("#penjualanLoad").show();
		penjualanListFiltered.map((list, index) => {
			const newItem = document.createElement("li");
			newItem.style.cursor = "pointer";
			newItem.onclick = function() {
				selectPenjualan(`${list.id}`,`${list.text}`);	
			};
			newItem.textContent = list.text;
			pL.appendChild(newItem)
		});
		setTimeout(() => {
			$("#penjualanLoad").hide();
			$("#layerLoad").hide();
	...
	function showList(){
		$("#add-select-penjualan-container").show();
		$("#penjualanFilter").focus();
	}

	function filterPenjualan(){
		const text = penjualanFilter.value.toString().trim();
		if (text.length > 3) {
			penjualanListFiltered = penjualanList.filter((list)=>list.text.toLowerCase().includes(text.toLowerCase()));
			isFilterMore = true;
		}else if(isFilterMore){
			penjualanListFiltered = penjualanList;
			isFilterMore = false;
		}
	...
	function selectPenjualan(id, text){
		console.log(id, text);
		selectedPenjualan.textContent  = text;
		penjualanId.value=id;
		$("#add-select-penjualan-container").hide();
