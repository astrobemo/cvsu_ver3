	...
	<div class='container'>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('retur_jual/penjualan_list_retur')?>" class="form-horizontal" id="form_add_data" method="post">
			                    	...
			                    	<div id='add-select-customer'>
			                    		<select name="penjualan_id" class='form-control' id="penjualan_id_select">
			                				...
			                				<?foreach ($penjualan_list as $row) { ?>
				                    			<option value="<?=$row->id?>"><?=$row->no_faktur_lengkap;?> - <?=($row->customer_id != 0 ? $row->nama_customer : $row->nama_keterangan)?></option>
				                    		<? } ?>
				                    	</select>
		...
		<div class="modal fade" id="portlet-config-edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('retur_jual/retur_jual_list_update')?>" class="form-horizontal" id="form_edit_data" method="post">
							...
							<div class="form-group">
			                    <label class="control-label col-md-3">Invoice Sumber<span class="required">
			                    * </span>
			                    </label>
			                    <div class="col-md-6">
	                    			<input disabled name='no_faktur_lengkap' class='form-control' value="<?=$no_faktur_lengkap;?>" >
			                    </div>
			                </div> 
			                ...
			                <div class="form-group customer_section">
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-6">
									<input type="text" hidden name="id" value="<?=$retur_jual_id?>">
									<input class="form-control" name="nama_keterangan" value="<?=$nama_customer?>" disabled />
			                    </div>
			                </div>
		...
		<div class="modal fade" id="portlet-config-detail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('retur_jual/retur_jual_list_detail_insert')?>" class="form-horizontal" id="form_add_barang" method="post">
			                    ...
			                    <div class="col-md-6">
			                    	<select name="barang_id_source" class='form-control input1' id='barang_id_select'>
		                				...
		                				<?$i=0;foreach ($retur_barang as $row) { ?>
			                    			<option value="<?=$row->id?>" data-index="<?=$i?>" data-toko="<?=$row->toko_id;?>"><?=$row->nama_jual;?> <?=$row->warna_jual;?> - Rp <?=number_format($row->harga_jual,"0",",",".");?></option>
			                    		<? $i++;} ?>
			                    	</select>
									<input type="text" name='barang_id' id="barang_id_add" hidden>
									<input type="text" name='warna_id' id="warna_id_add" hidden>
									<input type="text" name='toko_id' id="toko_id_add" hidden>
			                ...
			                <div class="form-group">
			                    ...
			                    * </span>
			                    ...
			                    <div class="col-md-6">
									<input type="text" class='amount_number form-control' id="hargaJualAdd" name="harga"/>
						...
						<table width="100%">
							<tr>
								...
								<td style="vertical-align: top; text-align:center; min-width:200px; padding-top:20px">
									<p style="font-size:1.2em">QTY JUAL</p>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						...
						<form hidden='hidden' action="<?=base_url()?>retur_jual/retur_jual_qty_update" id='form-qty-update' method="post">
							<input name='id'>
		...
		<div class="modal fade" id="portlet-config-pin" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-body">
						<form action="<?=base_url('retur_jual/retur_jual_request_open');?>" class="form-horizontal" id="form-request-open" method="post">
							<h3 class='block'> Cari Faktur</h3>
					...
					<div class="portlet-body">
						...
						<table class="table table-hover table-striped table-bordered" id="general_table">
							...
							<tbody>
								...
								foreach ($retur_detail as $row) { ?>
									<tr>
										...
										<td>
											...
											<span class='qty'><?=(float)$row->qty;?></span>
										</td>
										<td>
								...
								<span id="g-total"><?=number_format($g_total,'0',',','.');?></span>

								<tr class='subtotal-data'>
									<td colspan='6' class='text-right'><b>POTONGAN HARGA</b></td>
									<td><b><input <?=($status != 1 ? 'disabled' : '') ?> style="width:100px" type="text" id="potonganInput" class="amount_number" onchange="updatePotonganHarga()" value="<?=number_format($potongan_harga,'0',',','.');?> "> </b> </td>
									<td class='hidden-print'></td>
								</tr>

								<?=$g_total -= $potongan_harga;?>
								<tr class='subtotal-data'>
									<td colspan='6' class='text-right'><b>TOTAL</b></td>
									<td><b class='total' class="g-total-show"><?=number_format($g_total,'0',',','.');?> </b> </td>
									<td class='hidden-print'></td>
								</tr>
							</tbody>
						</table>
								...
								<td style="font-size: 1.2em;">
									...
									<div class="col-xs-8">
										<?if ($customer_id != 0 && $customer_id != '') {?>
											<label><input <?=$disabled?> type="radio" name="retur_jual_bayar" <?=($tipe_bayar==5 ? 'checked' : '')?> onclick="updateBayar('5')" value="5" id="bayar-1">Pengurangan Piutang</label><br/>	
										<?}?>
										<label><input <?=$disabled?> type="radio" name="retur_jual_bayar" <?=($tipe_bayar==2 ? 'checked' : '')?> onclick="updateBayar('2')" value="2" id="bayar-2">CASH</label>	<br/>
										<label><input <?=$disabled?> type="radio" name="retur_jual_bayar" <?=($tipe_bayar==4 ? 'checked' : '')?> onclick="updateBayar('4')" value="4" id="bayar-3">TRANSFER</label> <br/>
										<textarea <?=$disabled?> name="notes_bayar" id="notes_bayar" cols="30" rows="5" onchange="updateBayar('4')" style="<?=($tipe_bayar != 4 ? 'display:none': '')?>" ><?=$notes_bayar?></textarea>
									</div>
								</td>
								<td style='vertical-align:top;font-size:2.5em;' class='text-right'>
									<table style='float:right;'>
										<tr style='border:2px solid #c9ddfc'>    
											<td class='padding-rl-25' style='background:#c9ddfc'>TOTAL</td>
											<td class='padding-rl-10'>
												<b>Rp <span class='total_bayar g-total-show'style=''><?=number_format($g_total,'0','.',',');?></span></b>
											</td>
										</tr>
						...
						<hr/>
						<div>
							<button type='button' style="<?=($status != 1 ? 'display:none' : '');?>" class='btn btn-lg red hidden-print btn-close'><i class='fa fa-lock'></i> LOCK </button>
			                <a style="display: none;" <?=$disabled;?> href="<?=base_url();?>retur_jual/retur_jual_print?retur_jual_id=<?=$retur_jual_id;?>" target='_blank' class='btn btn-lg blue btn-print hidden-print'><i class='fa fa-print'></i> Print </a>
						</div>
					</div>
...
let hargaSource;
const retur_jual_id = "<?=$retur_jual_id?>";
...
const tokoIdAdd = document.querySelector("#toko_id_add");
...
const potonganInput = document.querySelector("#potonganInput");
const pembayaran = document.querySelector(`[name="retur_jual_bayar"]`);
const gTotal = document.querySelector('#g-total');

var tipe_bayar = "<?=$tipe_bayar?>";
...
jQuery(document).ready(function() {
    ...
    $('#customer_id_select, #customer_id_select_edit, #penjualan_id_select').select2({
        allowClear: true
    });
    ...
    $("#search_no_faktur").select2({
        ...
        query: function (query) {
            ...
            var data_st = {};
			var url = "retur_jual/get_search_no_faktur_jual";
    ...
    <?if ($retur_jual_id != '') {?>
    	$('.btn-close').click(function(){
			...
			if (tipe_bayar == '') {
				bootbox.alert("mohon isi cara pelunasan retur");
				return;
			}
	    	window.location.replace(baseurl+'retur_jual/retur_jual_list_close?id='+retur_jual_id);
	    });
    <?}?>
...
//====================================get harga jual barang====================================
    ...
    $('#barang_id_select').change(function(){
		...
		const tokoId = barangIdSource.find(":selected").attr('data-toko');
	    ...
	    // 	var url = "retur_jual/get_latest_harga";
    ...
    $('.btn-cek-harga').click(function(){
    	...
    	var url = 'retur_jual/cek_history_harga';
    	if (data['barang_id'] != '') {
    		var tbl = '<table>';
...
//====================================qty edit manage=============================    
	...
	$(".btn-brg-edit-save").click(function(){
		...
		var url = 'retur_jual/retur_jual_qty_update';
		ajax_data_sync(url,data).done(function(data_respond  /*,textStatus, jqXHR*/ ){
			if (data_respond == 'OK') {				
...
//====================================update harga=============================    
	...
	$('#general_table').on('change','[name=harga]', function(){
		...
		data['harga'] = $(this).val();
		var url = "retur_jual/update_retur_detail_harga";
...
//====================================btn save=============================    
    ...
    $('.btn-edit-save').click(function(){
    	var retur_type_id = $('#retur_type_edit').val();
    	if (retur_type_id != '') {
    		if (retur_type_id == 1 || retur_type_id == 2 ) {
    			if($('#form_edit_data [name=customer_id]').val() != ''){
    				$('#form_edit_data').submit();
    			}else if(retur_type_id == 1){
    				notific8('ruby','Mohon pilih customer');
    			}else if(retur_type_id == 2){
    				$('#form_edit_data').submit();
				}
    		}else{
    			$('#form_edit_data').submit();
	...
	$('#general_table').on('click','.btn-detail-remove', function(){
		...
		bootbox.confirm("Yakin mengahpus item ini?", function(respond){
			if (respond) {
				var data = {};
				data['id'] = ini.find('.id').html();
				var url = 'retur_jual/retur_list_detail_remove';
				ajax_data_sync(url,data).done(function(data_respond  /*,textStatus, jqXHR*/ ){
					if (data_respond == "OK") {
						ini.remove();
						update_table();
...
async function updateBayar(tipe){
	if (tipe != 4) {
		$("#notes_bayar").hide();
		$('#notes_bayar').val('');
	}else{
	...
	const notes_bayar = $('#notes_bayar').val();

	const response = await fetch(baseurl+"retur_jual/retur_jual_bayar_update",{
		method:"POST",
		headers:{
			'Content-Type': 'application/x-www-form-urlencoded'
		},
		body:`id=${retur_jual_id}&tipe_bayar=${tipe}&notes_bayar=${notes_bayar}`
	});
	tipe_bayar = await response.json();
	if (tipe_bayar.length != 1) {
		bootbox.alert("Error");	
	}else{
...
async function updatePotonganHarga(){

	const potongan = potonganInput.value;
	let resp = 'xxx';

	const response = await fetch(baseurl+"retur_jual/retur_jual_potongan_update",{
		method:"POST",
		headers:{
			'Content-Type': 'application/x-www-form-urlencoded'
		},
		body:`id=${retur_jual_id}&potongan_harga=${potongan}`
	});
	
	if (!response.ok) {
		console.error("Network error or server problem:", response.status, response.statusText);
		return;
	}
	...
	if (resp.length != 1) {
		...
		const total = reset_number_format(gTotal.innerHTML) - reset_number_format(potongan);
		gTotal.innerHTML = change_number_format(total);
		notific8("lime", `Potongan updated to ${total}`);
	}
